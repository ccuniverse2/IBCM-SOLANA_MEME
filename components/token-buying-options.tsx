        ...
        >
          {LOAN_LOTS.map((loanLot, loanLotIndex) => (
            <CustomRadio
              ...
              description={`(1 MINT IBCM = ${loanLot.token_price / 10 ** USDC_TOKEN_DECIMALS} USDC)`}
              value={loanLotIndex.toString()}
            >
              Buy {loanLot.token_quantity} MINT IBCM for {loanLot.token_quantity * (loanLot.token_price / 10 ** USDC_TOKEN_DECIMALS)} USDC
            </CustomRadio>
          ))}
        </RadioGroup>
